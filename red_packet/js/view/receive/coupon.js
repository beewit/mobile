...
new Vue({
    ...
    methods: {
                ...
                success: function (res) {
                    if (res.data.couponMaps) {
                        for (var d in res.data.couponMaps) {
                            if (that.receiveCouponIds.indexOf(res.data.couponMaps[d].id) > -1) {
                            ...
                            if (res.data.couponMaps[d].expire_time) {
                                ...
                                var now = new Date(common.formatDate(new Date(), "yyyy-MM-dd").replace(/\-/g, "\/"));
                                if (now > expireTime) {
        ...
        receive: function (id,index) {
            ...
            common.ajax({
                ...
                success: function () {
                    ...
                    Vue.set(that.List[index], 'receiveStatus', '已领取')
                },
                error: function (res) {
                    if (res && res.msg.indexOf('已领取') > -1) {
                        that.receiveCouponIds.push(id);
                        Cookies.set('receiveCouponIds', that.receiveCouponIds); 
                        Vue.set(that.List[index], 'receiveStatus', '已领取')
                    }
		...
		tapOpenDesc: function (index) {
			Vue.set(this.List[index], 'showDesc', !this.List[index].showDesc)
		}
    }
});